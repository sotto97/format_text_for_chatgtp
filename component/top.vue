			...
			<table class="w-6/12 px-2 text-center mx-auto">
				<tr>
					<td class="w-4/12 text-left">
						<label><input type="checkbox" class="h-5 w-5" v-model="qaSwitch"><span
								class="ml-2">質問内容</span></label>
					</td>
					<td class="w-4/12 text-left">
						<label><input type="checkbox" class="h-5 w-5" v-model="errorSwitch"><span
								class="ml-2">エラー文</span></label>
					</td>
					<td class="w-4/12 text-left">
						<label><input type="checkbox" class="h-5 w-5" v-model="envSwitch"><span
								class="ml-2">環境</span></label>
					</td>
				</tr>
				<tr>
					<td class="w-4/12 text-left">
						<label><input type="checkbox" class="h-5 w-5" v-model="codeSwitch"><span
								class="ml-2">ソースコード</span></label>
					</td>
					<td class="w-4/12 text-left">
				...
				<form ref="myForm">
					...
					<div class="mt-2">
						...
						<div class="mt-2" v-if="errorSwitch">
							...
							<div class="mt-2">
								<textarea v-model="errorContext" id="error-context" wrap="hard"
									@input="adjustTextareaHeight"
									class="block w-full rounded-md  p-1.5 text-gray-900 shadow-sm ring-1 ring-gray-300 resize-none"></textarea>
							</div>
						</div>
						<div class="mt-2" v-if="codeSwitch">
							<div class="flex items-center justify-between">
								<label for="password"
									class="block leading-6 text-gray-900">ソースコード（エスケープさせる処理めんどくさいの改行無し）</label>
							</div>
							<div class="mt-2">
								<textarea v-model="codeContext" id="code-context" wrap="hard" @input="adjustCodeareaHeight"
									class="block w-full rounded-md  p-1.5 text-gray-900 shadow-sm ring-1 ring-gray-300 resize-none"></textarea>
							</div>
						</div>
					</div>
